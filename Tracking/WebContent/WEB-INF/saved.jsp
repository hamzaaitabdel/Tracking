  		...
  		<c:out value=" ${erreur} "></c:out>
                 ...
                 <div class="card-body">
                                    <div class="icon-section">
                                        ...
                                        <div class="icon-section">
                                 <c:if test="${totalSaved == 0 }">
                <center><h2>Sie haben keine gespeicherten Dateien!</h2></center>
                <center><img src="./img/emptyty.png" alt=""></center>
                </c:if>
                <c:if test="${totalSaved != 0 }">
                            <div class="card alert">
                                <div class="card-header">
                                    <h4>Gespeicherte Dateien</h4>
                                </div>
              ...
              <div class="progress progress-striped active">
                <div class="progress-bar" role="progressbar" aria-valuenow="10" aria-valuemin="0" aria-valuemax="100" style="width: ${ (dossier.maxTache/ dossier.totalTache)*100}%">
                  <span class="sr-only">${ (dossier.maxTache/ dossier.totalTache)*100}% Komplett</span>
                </div>
              </div>
              </td>
              <td>
              <c:if test="${dossier.maxTache < dossier.totalTache}"> 
		<span class="badge badge-primary">Laufend</span> 
			  </c:if>
			  <c:if test="${dossier.maxTache == dossier.totalTache}">
		<span class="badge badge-success">Ausgefüllt</span>
			  </c:if>
			  </td>
			  <td><a  href="http://localhost:8082/Tracking/progression?tracking=${dossier.id_doc }"  ><button type="button" class="btn btn-dark btn-outline m-b-10 m-l-5">Verfolgen</button></a>
			  <a  href="http://localhost:8082/Tracking/deleteSaved?id_doc=${dossier.id_doc }&id_client=${ client.id_client }"  ><button type="button" class="btn btn-danger btn-outline m-b-10 m-l-5">Löschen</button></a></td>
                                            </tr>
                                            </c:forEach>
